...
async def add_product(message: types.Message):
    ...
    try:
        ...
        await message.answer(f"✅ Товар **{name}** добавлен в каталог!")
    except:
        await message.answer("⚠ Ошибка. Пример: `/add Сват_Пак 2500 Описание_тут`")
...
async def pay(callback: types.CallbackQuery):
    ...
    # Уведомление ОБОИМ админам
    for admin_id in ADMIN_IDS:
        if admin_id != 0:
            try:
                await bot.send_message(
                    admin_id, 
                    f"🔔 Клиент @{callback.from_user.username} хочет купить: {item[0]}"
                )
            except:
                pass
...
async def main():
    print("Бот ramaz666 и killedfear запущен!")
